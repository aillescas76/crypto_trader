defmodule CriptoTrader.Strategy.Experiment.DailyMaRegimeAsymmetric20260313 do
  @moduledoc """
  Daily 200-period SMA regime filter with asymmetric exit buffer (6% vs prior 3%).

  Micro-variant of DailyMaRegime20260313.

  Hypothesis: Increasing the exit buffer from 3% to 6% (requiring price to drop 6%
  below SMA200 before exiting) prevents whipsaw re-entries during choppy regime
  transitions. The prior strategy failed validation (-48.11%) due to 8 regime
  transitions in 2025 where price churned near the SMA200 threshold. A 6% buffer
  exceeds normal daily crypto volatility (3-5%) and should reduce transitions from
  8 to approximately 2-3, while preserving the training edge (196.7% vs BnH 66%).
  Expected to beat buy-and-hold on both training (pre-2025) and validation (2025+)
  splits with max_drawdown < 40%.

  Parameters from Step 4 micro-variant analysis:
  - ma_period: 200 (daily candles, per-pair own SMA) — unchanged
  - exit_buffer_pct: 0.06 (exit when daily close < SMA200 × 0.94) — changed from 0.03
  - entry: daily close > SMA200 (unchanged, asymmetric hysteresis)
  """
