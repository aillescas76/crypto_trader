defmodule CriptoTrader.Strategy.Experiment.MaGatedRotationSharpeRank3w20260314 do
  @moduledoc """
  Per-asset 20-week SMA gate + Sharpe-normalized momentum rotation with 3-week rebalance.

  Hypothesis: Extending the rebalance interval from 2w to 3w further reduces rotation
  churn in choppy markets. MaGatedRotationSharpeRank2w (best baseline: Val +25.82%)
  demonstrated that quality-filtered signals (Sharpe ranking) benefit non-additively from
  longer holding periods. Testing whether the quality×time interaction continues at 3w
  or if 2w is the optimal stopping point. Expected improvement: further churn reduction
  in 2025 choppy validation; expected cost: lower training PnL (fewer rebalances in
  strong 2023-2024 bull trends) and ~1 week slower exits in downtrends.

  Changes vs MaGatedRotationSharpeRank2w20260313 (current best baseline):
