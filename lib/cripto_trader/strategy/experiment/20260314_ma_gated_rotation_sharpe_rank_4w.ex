defmodule CriptoTrader.Strategy.Experiment.MaGatedRotationSharpeRank4w20260314 do
  @moduledoc """
  Per-asset 20-week SMA gate + Sharpe-normalized momentum rotation with 4-week rebalance.

  Hypothesis: Extending the rebalance interval from 3w to 4w further reduces rotation
  churn in choppy markets. MaGatedRotationSharpeRank3w demonstrated that the quality×time
  interaction is non-additive and has not plateaued at 3w (Val +33.81%, +50.74pp margin).
  Testing whether 4w continues the improvement. Sharpe-normalized ranking selects only
  persistent-trend assets — no drawdown penalty for holding longer, only compounding
  benefit as signals play out uninterrupted.

  Changes vs MaGatedRotationSharpeRank3w20260314 (current best baseline, Val +33.81%):
  - rebalance_interval: 3 weeks → 4 weeks
  ...
  # Signal function looks up at 4-week boundaries (a subset of weekly keys),
  # so every 4-week rebalance boundary resolves to a valid position target.
