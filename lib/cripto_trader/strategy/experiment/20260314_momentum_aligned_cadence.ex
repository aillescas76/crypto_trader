defmodule CriptoTrader.Strategy.Experiment.MomentumAlignedCadence20260314 do
  @moduledoc """
  Cross-asset momentum-alignment cadence + Sharpe-normalized rotation.

  Hypothesis: Replacing the vol-ratio regime discriminator with a cross-asset momentum-
  alignment counter fixes the structural flaw that prevented VolRegimeCadence from beating
  fixed 3w rebalance. VolRegimeCadence failed because short/long vol ratio cannot distinguish
  calm-trending from calm-ranging — both states produce similar low vol. What differs is
  DIRECTION: in a trending-calm market, most assets have positive 4w momentum simultaneously.

  Mechanism: At each weekly check, count how many of the 6 assets have positive 4w return.
  If >= 4 of 6 (>=67% aligned), use 1w cadence (momentum consensus is strong, rotate quickly
  to capture changes). If < 4 of 6, use 3w cadence (fragmented signals, let Sharpe quality
  play out without churn). Cadence switch is per-portfolio, not per-asset.

  Changes vs VolRegimeCadence8w20260314 (Train 277.71%, Val -5.31%, +11.6pp margin):
  - Discriminator: vol-ratio (short/long vol) → momentum-alignment count (>=4 of 6 positive)
  - Slow cadence: 2 weeks → 3 weeks (per SharpeRank3w validated optimum for choppy regimes)
  ...
  @quote_per_position 5_000.0
  @vol_floor 0.02
  # 4 of 6 assets must have positive 4w return
  ...
  # Returns the fraction of assets with positive 4w momentum.
  # Values: 0.0 (all negative) to 1.0 (all positive).
  # Threshold: >= 0.667 (4 of 6) = aligned bull market → 1w cadence.
